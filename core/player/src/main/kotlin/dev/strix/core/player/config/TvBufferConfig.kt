 ...
 * stalling again. A live feed is paced at ~1x so the buffer can't actually grow beyond
 * the cushion, keeping memory bounded despite the high [maxBufferMs].
 *
...
data class TvBufferConfig(
    ...
    companion object {
        /**
         * Lighter profile for the home preview/fullscreen player: a smaller first frame
         * ([bufferForPlaybackMs]) so a zap shows fast, and a lower memory ceiling
         * ([maxBufferMs]) for the low-RAM TV, while still cushioning jitter.
         */
        val preview =
            TvBufferConfig(
                minBufferMs = 15_000,
