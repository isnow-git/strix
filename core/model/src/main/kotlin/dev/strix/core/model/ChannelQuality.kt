...
/**
 * Splits a channel name into a quality label and a quality-independent base key, so
 * "BeIN Sports 1 FHD" and "BeIN Sports 1 HD" group under the same channel.
 *
 * Pure and allocation-light; runs once per channel at import (never on a scroll path).
 */
object ChannelQuality {
    ...
    /** Extracts quality label, grouping base key and time-shift from a raw [name]. */
    ...
    /**
     * The grouping key joining variants of the same channel: the provider EPG id when
     * present (strongest signal), else the normalized name; always split by time-shift
     * so a delayed feed never groups with the live one.
     */
    fun groupKey(
        epgChannelId: String?,
        info: QualityInfo,
     ...
     * every variant of one channel shares it — including a "+1" feed with no EPG id
     * and the live feed that has one. Used to inherit an EPG source from a sibling.
     */
    fun epgBaseKey(info: QualityInfo): String = "n:${info.baseKey}"

    /**
     * A clean display name: strips the country prefix, quality, codec and junk symbols
     * so a row reads "TF1" instead of "FR - TF1 HEVC". A time-shift is kept (as " +1")
     * so a delayed feed stays distinguishable from the live one. Computed once at
     * import (stored on the row), never on the scroll path. Pass an already-parsed
     * [info] to avoid re-running [parse] on [name].
     */
    fun displayName(
        name: String,
