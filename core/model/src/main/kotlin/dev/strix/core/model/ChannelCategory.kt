...
/**
 * Fallback classifier from name + provider category, used only when iptv-org has no
 * match. Keyword lists are kept tight (strong, unambiguous terms) so an unknown
 * channel lands in [ChannelCategory.General] rather than a wrong bucket.
 */
object ChannelClassifier {
    // Keywords are diacritics-folded once at class load, not per channel: the rule
    // table is constant, so normalizing it on every classify() call (once per
    // unmatched channel, i.e. most of them) was pure waste.
    ...
    /** Best-effort category for a channel from its [name] and optional [providerCategory]. */
    fun classify(
        name: String,
