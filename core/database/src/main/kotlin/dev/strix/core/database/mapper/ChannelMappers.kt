...
import dev.strix.core.model.ChannelQuality

/**
 * Entity <-> domain mapping. Kept as a single, direct transformation (no intermediate
 * DTO) to avoid per-item allocations on list scroll.
 */
 ...
 * playlist order is preserved without a runtime sort. `rowid` is left at its default
 * so SQLite assigns it (or reuses it on conflict-replace by channelId).
 *
 * The grouping fields ([ChannelEntity.baseKey], rank, label) are derived here from
 * the name; [ChannelEntity.isPrimary] is left false and set in a single pass after
 * the import (see `ChannelDao.finalizeGroups`).
 */
