...
interface EpgProgrammeDao {
    ...
    /** Programmes overlapping the window [fromSec, toSec) for a channel, earliest first. */
    @Query(
        "SELECT * FROM epg_programmes WHERE normChannelId = :normChannelId " +
            "AND stopSec > :fromSec AND startSec < :toSec ORDER BY startSec ASC",
    )
    suspend fun programmesInRange(
        normChannelId: String,
        fromSec: Long,
        toSec: Long,
    ): List<EpgProgrammeEntity>
