...
/**
 * Fetches iptv-org's community channel catalogue to classify channels accurately by
 * their canonical id (the same xmltv id we already use), instead of guessing from the
 * name. Falls back to the keyword classifier when a channel isn't found.
 */
class IptvOrgClient
    @Inject
    constructor(
        ...
        fun categoryMap(): Map<String, String> {
            ...
            return client.newCall(request).execute().use { response ->
                ...
                // channels.json is multi-MB; decode off the stream rather than buffering
                // the whole payload into a String first.
