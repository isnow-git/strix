 ...
 * passes [keep] are emitted, so ingestion stays bounded to the user's channels even
 * though a guide holds thousands.
 */
class XmltvParser {
    /**
     * Parses [input] and calls [onProgramme] for each kept programme. The callback is
     * `suspend` so the caller can flush each batch straight to storage from inside the
     * stream, keeping ingestion O(batch) instead of buffering the whole guide. This
     * never builds a list itself.
     */
    ...
    // XMLTV time: "20260601181000 +0200" (or without offset → treated as UTC). Parsed
    // by hand straight to epoch seconds: this runs twice per programme (tens of
    // thousands of times on a full guide), so a SimpleDateFormat per call — which
    // builds a Calendar and re-parses its pattern every time — would be the hot spot.
