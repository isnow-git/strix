...
import dev.strix.core.model.isSeparatorChannel
 ...
 *
 * Reads come straight from Room (paging + flows); a refresh streams the playlist over
 * OkHttp, parses it line by line, and writes it to Room in batches ([PlaylistImporter])
 * so a large playlist is never held whole in RAM. The Xtream path resolves channels
 * through [XtreamClient] instead of parsing M3U.
 *
...
class ChannelRepositoryImpl
    ...
    constructor(
        ...
        override fun pagedChannels(
            ...
            // scrollToItem() straight to any row (even far down) and Paging loads only
            // the window there. The null rows themselves are free.
            //
            // jumpThreshold makes a far jump invalidate and reload *once* at the target
            // position, instead of appending every page in between (which on a ~10k list
            // meant ~125 sequential queries — the multi-second stall).
            return Pager(
                config =
                    PagingConfig(
                        pageSize = PAGE_SIZE,
        ...
        private companion object {
            ...
            // Big enough that ordinary row-by-row scrolling never triggers a jump, small
            // enough that a keypad zap across the catalogue does.
