 ...
 * `OFFSET`, which re-reads every row before the window — fine near the top but janky
 * deep in a ~10k-row list. Placeholders ([LoadResult.Page.itemsBefore] /
 * [LoadResult.Page.itemsAfter]) let the UI jump straight to any row.
 *
 * Invalidated like Room's own sources: it observes the `channels` table and reloads
 * when an import rewrites it.
 */
