...
import dev.strix.core.model.epg.normalizeEpgId
 ...
 *
 * 1. **Authoritative XMLTV** (a trusted public guide), [refresh]ed into Room for the
 *    user's channels and matched by a normalized id — this corrects the provider's
 *    wrong/missing entries.
 * 2. **Provider fallback** (Xtream) when the XMLTV has nothing for a channel, with id
 *    inheritance from a live sibling and time-shift correction.
 *
...
class EpgRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun timeline(
            ...
            withContext(dispatchers.io) {
                val row = channelDao.findByChannelId(channel.id.value) ?: return@withContext emptyList()
                // Time-shifted feed: query the guide at shifted times, then map results back
                // to wall-clock so they sit correctly on the timeline's clock axis.
                val offsetSec = (row.timeshift.takeIf { it > 0 } ?: 0) * SECONDS_PER_HOUR
                val epgId = row.epgChannelId ?: channelDao.epgSibling(row.epgBaseKey)?.epgChannelId
                val norm = epgId?.let(::normalizeEpgId)?.takeIf { it.isNotEmpty() } ?: return@withContext emptyList()
                epgDao
                    .programmesInRange(norm, fromSec - offsetSec, toSec - offsetSec)
                    .map { entity ->
                        EpgProgramme(
                            title = entity.title,
                            startEpochSec = entity.startSec + offsetSec,
                            endEpochSec = entity.stopSec + offsetSec,
            ...
            withContext(dispatchers.io) {
                ...
                try {
                    ...
                    okHttpClient.newCall(request).execute().use { response ->
                        ...
                        // Stream the guide straight into Room in fixed-size batches: the
                        // buffer never holds more than INSERT_BATCH rows, so the whole
                        // guide (thousands of programmes) is never in RAM at once. The old
                        // guide is cleared only when the first batch is ready, so a
                        // failed/empty download leaves the previous one intact.
